...
namespace sy
{
   ...
   // !! --> When to : public Destructor <-- 
   // # when there isnt one already
   // # also library is assumed to be using Destructor correctly
   ...
   struct Time_tracker {
      ...
      bool Reset (sy::System_context* sys)
      {
      ...
      double GetDt (sy::System_context* sys)
      {
	...
	//
	// Graphics_display - roughly a graphics device
	class Graphics_display
		{
	public: 
   ...
   protected: 
	...
	public: 
		...
		// 
		virtual int		Initialize	   (System_context*) = 0; 
		virtual int		Deinitialize   (System_context*) = 0; 
		virtual int		Update	      (System_context*) = 0; 
   protected: 
		RT_Task () {
      }
   ...
   //
   // compose
   class RT_window_listener : public RT_Task, public Window_listener, public cx::Destructor 
      {
   protected: RT_window_listener () {}
      }; 
   //
   //
   int            Run_realtime_task       (System_context* sys, RT_Task* rtt); 
   System_context* Create_context (); 
