...
struct SPUX_SDL2_VK_window : public GraphicsWindow, public Destructor
{
   SPUX_SDL2_VK_window  (const std::string& title, int x, int y, int wd, int ht, size_t flags);
   virtual ~SPUX_SDL2_VK_window ();  


	// DeviceSurface 
   virtual bool	SetupDisplay   (); 
   ...
   //SDL_WINDOW_VULKAN; 
   bool                      display_setup ;
   Ref<SDL_Window>::Type window;

};



SPUX_SDL2_VK_window ::SPUX_SDL2_VK_window (const std::string& title, int x, int y, int wd, int ht, size_t flags)
   : display_setup (false)
   , window        (nullptr)  
{
   window = nullptr;
   
   //const size_t numbits = sizeof(GraphicsWindow::BIT_FLAGS) * 8;
   Uint32 create_flags  = SDL_WINDOW_VULKAN;

   assert (GraphicsWindow::VULKAN_BIT & flags);
   

   if (GraphicsWindow::FULLSCREEN_BIT & flags)
      create_flags |= SDL_WINDOW_FULLSCREEN ;

   window.reset (SDL_CreateWindow (title.c_str (), x, y, wd, ht, create_flags), SDL_DestroyWindow); 
   if (window)
   {
      int setupres = SetupDisplay (); 
   }
   else
   {
      assert (false); 
...
SPUX_SDL2_VK_window ::~SPUX_SDL2_VK_window ()
{

}

// DeviceSurface 
bool SPUX_SDL2_VK_window::SetupDisplay () 
{ 
   ...
   if (window)
   {
      SDL_bool create_res = SDL_Vulkan_CreateSurface (window.get(), instance, &surf);
...
void	SPUX_SDL2_VK_window::Present () 
{
...
size_t SPUX_SDL2_VK_window::Get_WindowID () { return 0; } 

void SPUX_SDL2_VK_window::Show (bool show)		
{
   if (window)
   {
      (show ? SDL_ShowWindow : SDL_HideWindow) (window.get());    
   }
} 

void SPUX_SDL2_VK_window::SetPos (int x, int y)	{ SDL_SetWindowPosition (window.get(), x, y);  } 
void SPUX_SDL2_VK_window::Resize (int x, int y)	{ SDL_SetWindowSize(window.get(), x, y); } 


class SDL_window : public spux::GraphicsWindow, public spux::Destructor
{
...
public:    
   ...
   //
   //
   SDL_window (const std::string& title, spux::WindowListener* ls, int wd, int ht, uint32_t sdl_flags, uint32_t sy_flags) 
	: win_(SDL_CreateWindow(title.c_str(), Window::Default_X_Pos, Window::Default_X_Pos, wd, ht, sdl_flags), SDL_DestroyWindow)
	, flags(sy_flags)
   {
      //SDL_Rect rect; 
   ...
   {
#if 0
	   if (flags & spux::GRAPHICS_WINDOW_OPENGL)
	   {
...
int Init_SPUX_SDL2 ()
{
   static const Uint32 kSDL_init_flags = SDL_INIT_AUDIO 
                                       | SDL_INIT_VIDEO 
                                       | SDL_INIT_EVENTS 
                                       | SDL_INIT_TIMER;

   return SDL_Init (kSDL_init_flags); 
}
...
//
//
struct SDL_system : public spux::System 
   { 
...
public:
   ...
   bool is_quit ;   // 
   std::map<SDL_Window*, spux::WindowListener*> map_window_2_listener; 


   virtual Ref<GraphicsWindow>::Type CreateGraphicsWindow (
      Ref<WindowListener>::Type listener, 
      ...
      size_t                        flags)
   {
      ...
      Ref<GraphicsWindow>::Type (new SPUX_SDL2_VK_window (title, posx, posy, wd, ht, flags));
...
//		  GRAPHICS_WINDOW_OPENGL = (0x1 << 1),
   ...
   virtual void PollInput (Mouse_state& m, Keyboard_state& k)
      {
         // mouse
	...
	virtual bool DispatchEvents ()
   {
      ...
      case SDL_WINDOWEVENT: 
         if (WindowListener* ls = map_window_2_listener[SDL_GetWindowFromID (event.window.windowID)] )
...
Ref<System>::Type spux::SystemContext ()
{

   static Ref<System>::Type sys (nullptr);

   if (!sys)
   {
      sys.reset(new SDL_system); 
   }
