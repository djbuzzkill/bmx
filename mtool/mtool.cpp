...
bool ECDSA_Sign (SECzy::Signature& sig, const SECzy::digest32& zbin, const SECzy::PrivateKey& privk) {
  ...
  // kG = R
  FE_t
    e    = dr(F->New_bin(std::data(privk.sec), 32, false)),
...
//         return Signature(r, s)
  ...
  // BEGIN SUS REGION
  // s = (z+re)/k
  // F->Mul (tmp, pt::x(pm[R]), e);
  // F->Add (snum, z, tmp);
  // F->MulM (stmp, snum, k_inv, em[n]);
  // // END SUS REGION

  // MEBE REPLACE SUS END
  F->SMul (tmp, pt::x(pm[R]), e);
  F->SAdd (snum, z, tmp);
  F->MulM (stmp, snum, k_inv, em[n]); 
  // MEBE REPLACE SUS END
  
  // F->Print( "R.x", pt::x(pm[R]));
  // F->Print( "s", s);
...
// bool ECDSA_Verify (ffm::el::map& em, ffm::pt::map& pm, ffm::FEConPtr F, ffm::ECConRef EC, const SECzy::Signature& sig, const SECzy::PublicKey& pubk, const af::digest32& zhash) {
  ...
  if (true) {
    ...
    SECzy::PrivateKey privk; { // init private key
      ...
      ffm::copy_BE (privk.sec, sec_bytes);
      // print_digest (privk.sec);

      ffm::FE_t reinit = F->New_bin( std::data(privk.sec), privk.sec.size(), false); 
    ...
    SECzy::PublicKey pubk; {
      ...
      ffm::FE_t e = F->New_bin (std::data(privk.sec), 32, false);
      em["e"] = e;
    ...
    // print_bn ("sig.s" , sig.s);
    // print_bn ("sig.r", sig.r); 
    {
